...
void JsonWriterTest::writeIntTest(){
    classUnderTest->write(QStringLiteral("test01"), 5);
    QVERIFY((*json)["test01"].toInt() == 5);
}

void JsonWriterTest::writeDoubleTest(){
    classUnderTest->write(QStringLiteral("test02.01"), 5.6);
    classUnderTest->write(QStringLiteral("test02.02"), -1.337);
    QVERIFY((*json)["test02.01"].toDouble() == 5.6);
    QVERIFY((*json)["test02.02"].toDouble() == -1.337);
}

void JsonWriterTest::writeStringTest(){
    classUnderTest->write(QStringLiteral("test03"), QStringLiteral("hallo"));
